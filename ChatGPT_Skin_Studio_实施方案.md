...
> 状态：MVP 与 Sparkle 2 集成已完成，正在准备 `v0.1.0 Public Beta` 签名、公证与远程发行。
...
- 真实 ChatGPT App 中通过截图、键盘、VoiceOver、resize 和清理回退验收。
...
当前常规单元与合同测试可独立运行；真实 App E2E、完整 visual diff、VoiceOver 和多状态手工验收仍是完整产品验收门禁，不能以编译或单元测试通过替代。用户明确要求本次发版期间不重启 ChatGPT，因此 `v0.1.0 Public Beta` 不运行会中断当前会话的 Production E2E，并必须在 Release Notes 中披露该限制。
